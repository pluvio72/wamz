...
import { getSeriesQueue } from "../../../api";
...
export default function SeriesInfo({ item }: Props) {
  ...
  const [state, setState] = useState<"editing" | "new" | "saved">(
    item.new ? "new" : "saved"
  );
  ...
  const watchEpisode = (episodeIdx) => {
    getSeriesQueue(dispatch, data.seasons[selectedSeason], episodeIdx);
    router.push('/video')
  };
    ...
    <div>
      <div className="flex flex-col h-full">
        <p className="text-xl font-medium mt-4 ml-4 mb-4">
          <span className="flex items-center justify-between">
            <span className="text-2xl">
              {data.seasons[selectedSeason].name}
            </span>
            {state !== "editing" && (
              <Dropdown
                items={[
        ...
        {data.seasons[selectedSeason].episodes.map((episode, index) => (
          ...
          //   }
          // }}>
          <div className="bg-neutral-800 p-2 rounded-lg px-3 mb-1 flex items-center cursor-pointer transition hover:bg-neutral-700" onClick={() => watchEpisode(index)}>
            {state === "editing" ? (
              <HiXCircle
                className="mr-3"
                color="red"
            ...
            <span className="ml-auto font-light text-sm">
              {episode.duration}
            </span>
          </div>
          // </Link>
        ...
        {state === "editing" && (
          <Button color="gray" className="mt-2" onClick={onClickAddEpisode}>
            <span className="flex items-center justify-center">
              Add episode <HiPlus className="ml-2" />
            </span>
          </Button>
        )}
        {data.seasons.length > 1 && (
          <div className="mt-auto flex items-center justify-evenly mr-2">
            <HiChevronLeft
              className={clsx(
                selectedSeason === 0 && "text-neutral-500",
                "transition hover:text-neutral-400 cursor-pointer"
              )}
              onClick={goToPreviousSeason}
            />
            <HiChevronRight
              className={clsx(
                selectedSeason === data.seasons.length - 1 &&
                  "text-neutral-600",
                "transition hover:text-neutral-400 cursor-pointer"
              )}
